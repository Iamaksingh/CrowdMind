...
/**
 * Check if comment is relevant to thread summary
 * Returns relevance score 1-100
 */
export const checkRelevance = async (commentText, threadSummary, threadTitle) => {
  try {
//     const prompt = `
// You are evaluating whether a comment is relevant to a discussion thread.

// First, reason step-by-step internally about the following items:
// 1. What is the main intent and focus of the thread title?
// 2. What is the main intent and focus of the thread summary?
// 3. Whether the comment contributes to or deviates from that focus.

// Do NOT include the reasoning in the output JSON. Think privately and only return JSON.

// Thread Title: """${threadTitle}"""
// Thread Summary: """${threadSummary || 'No summary available'}"""
// Comment: """${commentText}"""

// Rate the relevance of the comment to the thread on a scale of 1–100 using this rubric:
// - 1–30: Completely off-topic or spam
// - 31–60: Somewhat related but mostly deviates
// - 61–80: Mostly relevant with minor tangents
// - 81–100: Fully relevant and directly contributes
...
// - If unsure, choose a lower score rather than invent relevance
// `;
...
/**
 * Fact-check comment for factual claims
 * Returns whether comment has claims and if they appear verified
 */
export const factCheckComment = async (commentText, threadContext) => {
  try {
    const prompt = `
You are a professional fact-checker. Your task is only to analyze factual claims without inventing information.
...
Then privately evaluate whether those claims are supported based on common world knowledge. If something is not known or cannot be confirmed, classify it as "unverifiable". Do NOT attempt to guess missing information.
...
/**
 * Run background analysis on a comment
 * Updates the thread document with analysis results
 */
...
/**
 * Queue comment analysis (fire-and-forget)
